...
class VerifiedFollowersTab extends ConsumerWidget {
  final bool isMe;
  final String username;

  const VerifiedFollowersTab({
    super.key,
    required this.isMe,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final asyncFollower = ref.watch(verifiedFollowersProvider(username));
      ...
      data: (either) => either.fold(
        ...
        (followers) => RefreshIndicator(
          onRefresh: () async {
            ...
            await ref.refresh(verifiedFollowersProvider(username));
          },
          child: Column(
            children: [
              Expanded(

...
final blockedAccountsProvider =
    FutureProvider<Either<Failure, List<UserModel>>>((ref) async {
      final repo = ref.watch(settingsRepoProvider);
...
final mutedAccountsProvider = FutureProvider<Either<Failure, List<UserModel>>>((
  ref,
) async {
  final repo = ref.watch(settingsRepoProvider);
  final user = ref.watch(currentUserProvider);
...
final mutedUsersNotifierProvider =
    StateNotifierProvider.autoDispose<MutedUsersNotifier, MutedUsersState>((
      ref,
    ) {
      final repo = ref.watch(settingsRepoProvider);
...
final blockControllerProvider = Provider((ref) {
  ...
  return (String username) => repo.blockUser(username, ref);
});

final unblockControllerProvider = Provider((ref) {
  final repo = ref.watch(profileRepoProvider);
  return (String username) => repo.unBlockUser(username, ref);
});

final muteControllerProvider = Provider((ref) {
  final repo = ref.watch(profileRepoProvider);
  return (String username) => repo.muteUser(username, ref);
});

final unMuteControllerProvider = Provider((ref) {
  final repo = ref.watch(profileRepoProvider);
  return (String username) => repo.unMuteUser(username, ref);
});

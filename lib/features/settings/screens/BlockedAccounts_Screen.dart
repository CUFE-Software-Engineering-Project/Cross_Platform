...
class BlockedAccountsScreen extends ConsumerWidget {
  ...
  Widget _blockedTile({required UserModel user, required VoidCallback onUnblock}) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 12.0),
      child: Row(
        ...
        (users) {
          if (users.isEmpty) {
            return const Padding(
              ...
              child: Text('No blocked accounts', style: TextStyle(color: Palette.textWhite)),
            );
          }
          return ListView.builder(
            itemCount: users.length,
            itemBuilder: (c, i) => _blockedTile(
              user: users[i],
              onUnblock: () async {

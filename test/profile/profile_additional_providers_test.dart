...
void main() {
  setUpAll(() {
    ...
    provideDummy<Either<Failure, List<TweetReplyModel>>>(
      Left(Failure('dummy')),
    );
  });
  ...
  group('Profile additional providers (mockito)', () {
    ...
    test(
      'updateProfileBannerProvider calls repo.updateProfileBanner and returns Right',
      () async {
        when(
          mockRepo.updateProfileBanner('user1', 'media1'),
    ...
    test(
      'updateProfilePhotoProvider calls repo.updateProfilePhoto and returns Right',
      () async {
        when(
          mockRepo.updateProfilePhoto('user1', 'media1'),
        ).thenAnswer((_) async => const Right(null));
    ...
    test(
      'deleteTweetProvider calls repo.deleteTweet and returns Right',
      () async {
        when(
          mockRepo.deleteTweet('tweet1'),
        ).thenAnswer((_) async => const Right(null));
    ...
    test(
      'profileCurrentSearchProvider returns search results on success',
      () async {
        ...
        when(
          mockRepo.profileCurrentSearch('query'),
        ).thenAnswer((_) async => Right(mockSearchResults));

        final container = ProviderContainer(
          overrides: [profileRepoProvider.overrideWithValue(mockRepo)],
        );

        final either = await container.read(
    ...
    test(
      'changeEmailProfileProvider calls repo.changeEmailProfile and returns Right',
      () async {
        when(
          mockRepo.changeEmailProfile('newemail@test.com'),
        ).thenAnswer((_) async => const Right(null));
    ...
    test(
      'verifyChangeEmailProfileProvider calls repo.verifyChangeEmailProfile and returns Right',
      () async {
        when(
          mockRepo.verifyChangeEmailProfile('newemail@test.com', '123456'),
        ).thenAnswer((_) async => const Right(null));
    ...
    test(
      'changePasswordProfileProvider calls repo.changePasswordProfile and returns Right',
      () async {
        when(
          mockRepo.changePasswordProfile('oldPass', 'newPass', 'newPass'),
        ).thenAnswer((_) async => const Right(null));
    ...
    test('tweetRepliesProvider returns replies on success', () async {
      ...
      when(
        mockRepo.getTweetReplies('tweet1'),
      ).thenAnswer((_) async => Right(mockReplies));
      ...
      final either = await container.read(
        tweetRepliesProvider('tweet1').future,
      );
      expect(either.isRight(), isTrue);
      either.fold((l) => fail('expected Right'), (r) => expect(r.length, 1));
    });

...
void main() {
  group('MutedUsersNotifier', () {
    ...
    test('fetchInitial populates users', () async {
      ...
      when(mockRepo.fetchMutedAccounts(limit: 10, cursor: null))
          .thenAnswer((_) async => Right(MutedUsersResponse(users: [u1], nextCursor: null, hasMore: false)));

      final notifier = MutedUsersNotifier(settingsRepo: mockRepo, pageSize: 10);
    ...
    test('loadMore appends users when hasMore true', () async {
      ...
      when(mockRepo.fetchMutedAccounts(limit: 10, cursor: null)).thenAnswer((_) async =>
          Right(MutedUsersResponse(users: [u1], nextCursor: 'c1', hasMore: true)));

      when(mockRepo.fetchMutedAccounts(limit: 10, cursor: 'c1')).thenAnswer((_) async =>
    ...
    test('reports error on fetch failure', () async {
      when(mockRepo.fetchMutedAccounts(limit: 10, cursor: null))
          .thenAnswer((_) async => Left(Failure('err')));
